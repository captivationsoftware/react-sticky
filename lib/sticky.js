...
var Sticky = React.createClass({
  ...
  shouldBeSticky: function() {
    return !this.isInBottomOffset() && this.pageOffset() >= (this.top() + this.pageOffset()) + this.props.topOffset;
  },
  /*
   * Returns true/false if there is bottomOffset and scroll position is within bottomOffset.
   */
  isInBottomOffset: function() {
    var offset = this.pageOffset();
    var topbottomOffset = document.documentElement.scrollHeight - this.props.bottomOffset - this.domNode.getBoundingClientRect().height;
    return ((this.props.bottomOffset > 0) && (offset >= topbottomOffset));
  },
  /*
  ...
  handleFrame: function() {
    if (this.hasUnhandledEvent || this.hasTouchEvent) {
      ...
      var isInBottomOffset = this.isInBottomOffset()
      this.nextState(shouldBeSticky, isInBottomOffset);
  ...
  componentDidMount: function() {
    ...
    if (this.refs['sticky-wrapper']) this.refs['sticky-wrapper'].style.height = this.domNode.getBoundingClientRect().height + 'px';
   ...
   */
  nextClassName: function(shouldBeSticky, isInBottomOffset) {
    var className = this.props.className;
    if (shouldBeSticky) {
      className += ' ' + this.props.stickyClass;
    }
    if (isInBottomOffset) {
      className += ' ' + this.props.stickyBottomClass;
    }
    return className;
  },
  /*
   ...
   */
  nextState: function(shouldBeSticky, isInBottomOffset) {
    var hasChanged = this.state.isSticky !== shouldBeSticky;
    this.setState({
      isSticky: shouldBeSticky,
      style: this.nextStyle(shouldBeSticky),
      className: this.nextClassName(shouldBeSticky, isInBottomOffset)
    });
   ...
   */
  render: function() {
    if (this.props.createShadowWrap) {
